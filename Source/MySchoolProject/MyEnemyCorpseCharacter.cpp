...
AMyEnemyCorpseCharacter::AMyEnemyCorpseCharacter()
{
	...
	staticMeshPlacer->SetupAttachment(RootComponent);
...
void AMyEnemyCorpseCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
...
void AMyEnemyCorpseCharacter::OnConstruction(const FTransform& Transform) {
...
bool AMyEnemyCorpseCharacter::ShouldTickIfViewportsOnly() const
{
	return true;
}
...
void AMyEnemyCorpseCharacter::PostInitializeComponents()
{
	...
	myCont = Cast<AMyCorpseEnemyAIController>(GetController());
	if (myCont) {
			...
			if (OtherComp->ComponentHasTag("PlayerCapsule")) {
				if (player) {
					if (!player->isRolling) {
						...
						UE_LOG(LogTemp, Warning, TEXT("HIT PLAYER AND HE TOOK DAMAGE"));
